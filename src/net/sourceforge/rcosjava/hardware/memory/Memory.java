 ...
 * </DD></DD>
 * 19/5/2001 Fixed it so that setFree creates a new memory segment.
 * </DD></DT>
 * <P>
...
public class Memory implements Serializable, Cloneable
{
  ...
  /**
   * Create a memory class with the size of DEFAULT_SEGMENT.  By default is
   * set to be free.
   */
  public Memory()
  {
  ...
  /**
   * Create a memory class with the pre-exisiting memory.  Writes the memory
  ...
  /**
   * Sets the memory to be unallocated and resets the memory segment to its
   * null value.
   */
  public void setFree()
  {
   ...
   *
   * @param the index in the memory block - assumed not bigger than maximum
   * size otherwise will throw an OutOfBounds exception.
  ...
  /**
   * Allows two memory blocks to be combined into one.  The total size will
   * be combined segment sizes of the first and the second memory segment.
   *
  ...
  /**
   * Write a value to the memory segment give an offset from which to use as
   * an index.
   *
  ...
  /**
   * The integer version of @see #write(int, short).  Will convert the int to
   * a short.
   */
  ...
  /**
   * Writes a memory block.  Assumes that the offset plus the length of the
   * new memory block does not exceed the maximum size of this memory segment.
   * No checking is done.
   *
